...
- store filename only in imports.source_file, not full path (#32)
- clean up lock file in FileLock.__exit__ (#31)
- add numpy and matplotlib as optional analytics dependencies (#30)
- use defusedxml for PubMed XML parsing (#29)
- strip redundant 'Generated by' footer from PDF — already in page footer *(#12)*
- larger h3 headers, add GitHub URL to PDF footer *(#12)*
- add nl2br extension so single newlines render as line breaks in PDF *(#12)*
- lazy-import WeasyPrint and set DYLD_FALLBACK_LIBRARY_PATH for macOS *(#12)*
- split Escape/Tab handlers in useFocusTrap to avoid double-call
- add import deps to CI, add gene_snp_map table to test fixture
- add triage deps to CI so TUI tests can run

### Documentation

- MH action roadmap implementation plan *(#24)*
- MH action roadmap design spec (#24)
- risk landscape timeline implementation plan *(#27)*
- risk landscape timeline + honest risk communication design spec (#27)
- PDF export implementation plan (#12)
- PDF export design spec (#12) — WeasyPrint approach
- evidence comparison implementation plan (#13)
- evidence comparison design spec (#13)
- keyboard navigation implementation plan (#19)
- keyboard navigation design spec (#19)

### Features

- preflight checks, 20-sample VCF, expanded guides *(onboarding)*
- render [[wikilinks]] as interactive gene links *(mental-health)*
- wire ActionRoadmap into MentalHealthDashboard *(#24)*
- add ActionRoadmap component with scoring and tests *(#24)*
- add ConfidenceDots, TimelineSection, and honest risk disclaimer *(#27)*
- add confidence scoring and timeline grouping to useRiskData *(#27)*
- add screenings to risk-landscape config *(#27)*
- add mental health PDF export to App.tsx *(#12)*
- add PDF export button to PGx panel *(#12)*
- add exportPdf() function to frontend *(#12)*
- add PDF export route POST /api/export/pdf *(#12)*
- add PDF renderer service with tests *(#12)*
- replace hardcoded pathway systems with config-driven architecture (#24, #28)
- integrate EvidenceComparison into VariantDrawer *(#13)*
- add EvidenceComparison component with tests *(#13)*
- add focus-visible indicator for table rows *(#19)*
- integrate useFocusTrap into CommandPalette *(#19)*
- integrate useFocusTrap into ChecklistSidebar *(#19)*
- integrate useFocusTrap into VariantDrawer *(#19)*
- integrate keyboard navigation into SNPTable *(#19)*
- add useTableKeyboardNav hook with tests *(#19)*
- add useFocusTrap hook with tests *(#19)*
- goal-centric filter pills in SNP Browser (#10)
- population frequency in VariantDrawer (#18), PDF export design (#12)
- add schizophrenia and autism PGC GWAS datasets (8 traits total)
- zygosity column in SNP table (#14), actionable filter tooltip (#16)
- close chat history gaps — rename, loading state, delete undo (#9)
...
- add CHANGELOG.md and git-cliff config for conventional commits

### Testing

- improve ActionRoadmap expand/collapse test coverage *(#24)*
